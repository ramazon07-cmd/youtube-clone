...
def video_detail(request, video_id):
    ...
    return render(request, 'videos/video_detail.html', {'video': video, "user_vote": user_vote})
        ...
        try:
            ...
            if custom_thumbnail and custom_thumbnail.startswith("data:image"):
                try:
                    ...
                    thumbnail_file_id = thumb_result["file_id"] 
...
def video_delete(request, video_id):
    ...
    try:
        delete_video(video.file_id, video.thumbnail_file_id)
        video.delete()  
...
def video_vote(request, video_id):
    ...
    if existing_vote:
        ...
        VideoLike.objects.create(user=request.user, value=value, video=video)
        if value == VideoLike.LIKE:
            video.likes += 1
        else:
            video.dislikes += 1
